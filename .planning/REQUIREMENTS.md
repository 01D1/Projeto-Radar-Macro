...
- [x] **FIN-01**: LTM (Last Twelve Months) financial aggregation computed for all watchlist tickers: revenue, EBITDA, net income, FCF, net debt — rolling sum of trailing 4 ITR quarters, reconciled against latest DFP *(completed: 03-01, 2026-05-11)*
- [x] **FIN-02**: Standard valuation multiples computed for each ticker: P/E, EV/EBITDA, P/BV, dividend yield, EV/Revenue; updated daily with latest price + latest LTM financials *(completed: 03-02, 2026-05-11)*
- [x] **FIN-03**: DCF fair value model per ticker: WACC derived from Selic + CDS Brazil + equity risk premium; FCF projections over 5 years; terminal value with configurable terminal growth rate; output includes fair value in BRL and upside/downside % vs current price *(completed: 03-03, 2026-05-11)*
- [x] **FIN-04**: DCF model validates inputs before running (terminal growth < WACC; WACC > 5%); fair value outside 0.1x–5.0x current price flagged as "FORA DO INTERVALO CONFIÁVEL" rather than displayed *(completed: 03-03, 2026-05-11)*
- [x] **FIN-05**: Bank/financial sector tickers routed to alternative financial model (NIM-based instead of EBITDA-based; ROE instead of ROIC; dividend discount model instead of FCF DCF) *(completed: 03-04, 2026-05-11)*
...
- [x] **INT-01**: AI investment thesis generated per ticker using Claude API (`claude-sonnet-4-6`) via `instructor` with enforced Pydantic schema: bull case, bear case, key drivers (3–5), risks (3–5), fair value BRL, methodology disclosure, positioning (COMPRAR/MANTER/VENDER), confidence (ALTA/MEDIA/BAIXA), rationale; output in Portuguese *(completed: 04-01/04-02, 2026-05-17)*
- [x] **INT-02**: All numbers in AI thesis injected from Financial Engine outputs — LLM performs synthesis only, never calculation; post-generation validation cross-checks thesis fair value against computed DCF (tolerance ±10%) *(completed: 04-02, 2026-05-17)*
- [x] **INT-03**: Input hash computed per thesis (DCF result + top multiples + macro snapshot + top 3 news headlines + signals); thesis regenerated only when input hash changes; max 2 regenerations per ticker per day *(completed: 04-02, 2026-05-17)*
- [x] **INT-04**: Thesis stored in database with version history; each version records input hash, generation timestamp, and what changed from prior version *(completed: 04-01/04-02, 2026-05-17)*
- [x] **INT-05**: Opportunity signals computed per ticker: price vs DCF fair value divergence (flag if >20% mispriced), momentum shift detection (MA crossover + fundamental backing), event-driven signal (IPE event + thesis impact) *(completed: 04-03, 2026-05-17)*
- [x] **INT-06**: Opportunity signals ranked across watchlist by conviction score; top 3 opportunities available as structured output for Telegram and dashboard *(completed: 04-03/04-04, 2026-05-17)*
...
| INT-01 | Phase 4 | Complete (04-01/04-02, 2026-05-17) |
| INT-02 | Phase 4 | Complete (04-02, 2026-05-17) |
| INT-03 | Phase 4 | Complete (04-02, 2026-05-17) |
| INT-04 | Phase 4 | Complete (04-01/04-02, 2026-05-17) |
| INT-05 | Phase 4 | Complete (04-03, 2026-05-17) |
| INT-06 | Phase 4 | Complete (04-03/04-04, 2026-05-17) |
| DEL-01 | Phase 5 | Partial (05-02: 4 pages rendered; full PDF content in 05-04) |
| DEL-02 | Phase 5 | Partial (05-02: freshness caption + TTL cache; complete after 05-04) |
| DEL-03 | Phase 5 | Pending (05-03) |
| DEL-04 | Phase 5 | Pending (05-03) |
| DEL-05 | Phase 5 | Partial (05-02: download button wired; full PDF sections in 05-04) |
